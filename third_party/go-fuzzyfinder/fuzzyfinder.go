...
func (f *finder) _draw() {
	...
	// Layout-dependent anchor rows. The prompt line is always on the very
	// bottom row. In the default LayoutBottom the header and number line sit
	// directly above the prompt and the item list grows upward from just below
	// them. In LayoutReverse the item list is anchored to the top of the
	// terminal and grows downward, with the number line and header following
	// the last rendered item.
	reverse := f.opt.layout == LayoutReverse
	hasHeader := len(f.opt.header) > 0

	headerRow := maxHeight - 2
	numberRow := maxHeight - 2
	if hasHeader {
		numberRow = maxHeight - 3
	}
	itemAreaHeight := numberRow - 1
	if itemAreaHeight < 0 {
		itemAreaHeight = 0
	}
	...
	if reverse {
		...
		if rendered > itemAreaHeight+1 {
			rendered = itemAreaHeight + 1
		}
		infoRow = rendered
	}
	for i, r := range fmt.Sprintf("%d/%d", len(f.state.matched), len(f.state.items)) {
		...
		f.term.SetContent(2+i, infoRow, r, nil, style)
	}
	...
	if hasHeader {
		...
		for _, r := range runewidth.Truncate(f.opt.header, maxWidth-2, "..") {
			style := tcell.StyleDefault.
				Foreground(tcell.ColorGreen).
				Background(tcell.ColorDefault)
			f.term.SetContent(2+w, hdrRow, r, nil, style)
			w += runewidth.RuneWidth(r)
		}
	}
}
...
func (f *finder) readKey(ctx context.Context) error {
	...
	// With LayoutBottom the list grows upward from the bottom, so later items
	// (higher indices) are drawn above earlier ones; with LayoutReverse the list
	// is anchored to the top and grows downward, so the index order is inverted
	// on screen. The step sign flips the scroll direction to match the visual
	// up/down arrow and page keys for the active layout.
	step := 1
	if f.opt.layout == LayoutReverse {
		step = -1
	}

	// moveSelection advances the cursor by delta items (delta already includes
	// the layout flip). y changes whenever it stays in range; cursorY changes
	// only while it stays within the visible item area, so the list scrolls
	// once the cursor reaches an edge.
	moveSelection := func(delta int) {
		ny := f.state.y + delta
		if ny >= 0 && ny < matchedLinesCount {
			f.state.y = ny
			nc := f.state.cursorY + delta
			if nc >= 0 && nc < min(matchedLinesCount, screenHeight-2) {
				f.state.cursorY = nc
			}
		}
	}

	pageMove := func(delta int) {
		delta *= pageScrollBy
		f.state.y = clamp(f.state.y+delta, 0, matchedLinesCount-1)
		maxCursorY := min(screenHeight-3, matchedLinesCount-1)
		f.state.cursorY = clamp(f.state.cursorY+delta, 0, maxCursorY)
	}
